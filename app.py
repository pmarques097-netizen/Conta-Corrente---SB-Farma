...
APP_STATE_SYNC = CLOUD_MODE and _truthy(APP_SECRETS.get('state_sync', 'true'))
...
try:
    st.sidebar.image(str(LOGO_BRANCO), width='stretch')
except Exception:
...
st.sidebar.markdown('---')
st.sidebar.caption('Versão 14.3 Cloud Definitivo')
...
if menu == 'Negociações':
            ...
            if modo_produto == 'Selecionar da base':
                ...
                if arq_prod is not None:
                    try:
                        ...
                        st.dataframe(df_prod_import.head(30), width='stretch')
    ...
    if aba_import == 'Importar compras Excel/CSV':
        ...
        if file:
            ...
            st.dataframe(df_preparado.head(30), width='stretch')
    ...
    if aba_import == 'Importar vendas SQL/Excel':
        ...
        if vendas_file is not None:
            try:
                ...
                st.dataframe(preparar_venda_sql(df_vendas).head(30), width='stretch')
    ...
    with st.expander('Atualizar / Recalcular', expanded=False):
        ...
        with col_upd:
            ...
            if st.button('🔄 Atualizar dados pela vigência', width='stretch'):
                try:
        ...
        with col_calc:
            ...
            if st.button('⚙️ Recalcular apuração agora', type='primary', width='stretch'):
                try:
    ...
    if (ap is None or ap.empty) and (ap_prod is None or ap_prod.empty):
        ...
        if ap_prod is not None and not ap_prod.empty:
            ...
            st.dataframe(show_prod, width='stretch')
    ...
    if not opcoes:
        ...
        with st.form('form_workflow'):
            ...
            ok = st.form_submit_button('Registrar etapa', width='stretch')
            if ok:
        ...
        if not hist.empty:
            ...
            if not view.empty:
                ...
                st.dataframe(view.sort_values('id', ascending=False), width='stretch')
            else:
        ...
        if entidades.empty:
            ...
            if ext.empty:
                ...
                for c in ['Crédito (R$)','Débito (R$)','Saldo (R$)']:
                    if c in show.columns:
                        show[c] = show[c].map(money)
                st.dataframe(show[cols_show], width='stretch', height=430)
                    ...
                    if det_lanc.empty:
                        ...
                        if acao_lanc == 'Visualizar':
                            ...
                            with st.form(f'form_editar_lancamento_visivel_{lanc_id_acao}'):
                                ...
                                salvar_edit2 = st.form_submit_button('Salvar alterações', width='stretch')
                                if salvar_edit2:
            ...
            with st.expander('Novo lançamento financeiro / baixa / glosa / abatimento', expanded=False):
                ...
                with st.form('form_lancamento_financeiro_extrato'):
                    ...
                    salvar = st.form_submit_button('Salvar lançamento financeiro', width='stretch')
                    if salvar:
                        if valor <= 0:
                            st.warning('Informe um valor maior que zero.')
            ...
            with st.expander('Editar ou excluir lançamentos avulsos', expanded=False):
                ...
                if avulsos.empty:
                    ...
                    st.dataframe(av_view[cols_av], width='stretch', height=260)
                    labels_av = []
                    mapa_av = {}
                    ...
                    with tabs_av[0]:
                        with st.form(f"form_editar_lanc_avulso_{int(row_av.get('id'))}"):
                            ...
                            salvar_edit_av = st.form_submit_button('Salvar alterações do lançamento', width='stretch')
                            if salvar_edit_av:
    ...
    if not opcoes:
        ...
        with st.form('form_documento'):
            ...
            ok = st.form_submit_button('Salvar documento', width='stretch')
            if ok:
                if arq is None:
    ...
    if negociacoes is None or negociacoes.empty:
        ...
        with abas[0]: st.dataframe(proximas[cols].sort_values('Dias para vencer'), width='stretch')
        with abas[1]: st.dataframe(vencidas[cols].sort_values('Dias para vencer'), width='stretch')
        with abas[2]: st.dataframe(sem_apuracao[cols], width='stretch')
        with abas[3]: st.dataframe(n[cols].sort_values('Dias para vencer'), width='stretch')
    ...
    if negociacoes.empty:
        ...
        with st.form('form_recebimento_v10'):
            ...
            salvar_rec = st.form_submit_button('Salvar recebimento', width='stretch')
            if salvar_rec:
                if valor_recebido <= 0:
    ...
    if negociacoes.empty:
        ...
        try:
            ...
            st.download_button('📄 Baixar prestação de contas em PDF', pdf_bytes, file_name=nome_arq, mime='application/pdf', width='stretch')
        except Exception as e:
            st.error(f'Não foi possível gerar o PDF: {e}')
        if st.button('Registrar etapa: Prestação de contas enviada', width='stretch'):
    ...
    with tab_credito:
        ...
        with st.form('form_novo_tipo_credito_parametros'):
            ...
            salvar_tipo = st.form_submit_button('Cadastrar tipo de crédito', width='stretch')
            if salvar_tipo:
        ...
        if tipos_df.empty:
            ...
            if row_tipo is not None:
                with st.form('form_editar_tipo_credito_parametros'):
                    ...
                    salvar_edit = col_a.form_submit_button('Salvar alteração', width='stretch')
                    inativar = col_b.form_submit_button('Inativar / excluir da lista', width='stretch')
                    reativar = col_c.form_submit_button('Reativar', width='stretch')
                    if salvar_edit:
    ...
    with tab_tipo_neg:
        ...
        if not tipos_neg_df.empty:
            ...
            st.dataframe(vis[cols].rename(columns={'nome':'Tipo','ativo':'Ativo','controla_produtos':'Produtos','faz_apuracao':'Apuração','gera_financeiro':'Financeiro','utiliza_faixas':'Faixas','utiliza_metas':'Metas','observacao':'Observação'}), width='stretch', hide_index=True)
        with st.form('form_tipo_negociacao_parametro'):
            ...
            if st.form_submit_button('Salvar tipo de negociação', width='stretch'):
        ...
        if tipos_ativos_para_inativar:
            ...
            if st.button('Inativar tipo selecionado', width='stretch'):
    ...
    if negociacoes.empty:
        ...
        if neg is not None:
            ...
            if ap_produtos is None or ap_produtos.empty:
                ...
                if resumo_cols:
                    st.dataframe(ap_resumo[resumo_cols].head(50), width='stretch')
            else:
                ...
                st.dataframe(ap_produtos[preview_cols].head(50), width='stretch')
            ...
            try:
                ...
                st.download_button('📄 Baixar relatório em PDF', pdf_bytes, file_name=nome_arq, mime='application/pdf', width='stretch')
            except Exception as e:
                msg = str(e)
    ...
    with tab_a1:
        if hist.empty:
            ...
            st.dataframe(show_hist, width='stretch')
    ...
    with tab_a2:
        st.dataframe(neg_aud, width='stretch')
    ...
    with tab2:
        compras = load_table('compras')
        st.dataframe(compras.tail(1000), width='stretch')
