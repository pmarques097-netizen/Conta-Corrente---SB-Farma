...
CLOUD_SQL_MAX_DAYS = int(os.environ.get('SB_FARMA_CLOUD_SQL_MAX_DAYS', '1') or '1')


def validar_periodo_sql_cloud(data_inicio, data_fim, operacao='consulta SQL'):
    """Protege o Streamlit Cloud contra consultas muito grandes no processo web.

    O Community Cloud encerra o app quando a thread principal fica ocupada por muito
    tempo ou consome muita memória. Por isso, no Cloud as cargas SQL precisam ser
    feitas por períodos curtos. Para carga histórica completa, rode local/servidor
    dedicado e suba o cache, ou use um job externo.
    """
    try:
        ...
        return
    dias = (fim - ini).days + 1
    if CLOUD_MODE and dias > CLOUD_SQL_MAX_DAYS:
        raise RuntimeError(
            f'No Streamlit Cloud, {operacao} foi bloqueada porque o período tem {dias} dias. '
            f'O sistema agora roda em lotes diários. Reduza o lote ou tente novamente.'
        )

HOURLY_SYNC_MINUTE = 5  # roda uma vez por hora, próximo ao minuto 05

st.set_page_config(page_title='SB Farma | Projeto Negociação v15.1 Streamlit Cloud', page_icon='💊', layout='wide')
...
def executar_script_postgres(host, port, database, user, password):
    ...
    conn = psycopg2.connect(host=host, port=port, dbname=database, user=user, password=password, connect_timeout=8, options='-c statement_timeout=120000')
    try:
...
def executar_script_vendas_postgres(host, port, database, user, password, data_inicio=None, data_fim=None):
    if data_inicio and data_fim:
        validar_periodo_sql_cloud(data_inicio, data_fim, 'atualização de vendas')
    try:
        psycopg2 = _import_postgres_driver()
    except Exception as exc:
        raise RuntimeError('Instale um driver PostgreSQL compatível para importar direto do banco PostgreSQL.') from exc
...
def executar_script_entradas_postgres_periodo(host, port, database, user, password, data_inicio, data_fim, origem='CACHE_SQL_ENTRADAS_SB', substituir_periodo=True):
    validar_periodo_sql_cloud(data_inicio, data_fim, 'atualização de entradas')
...
def dividir_periodos_em_lotes(data_inicio, data_fim, dias_por_lote=1):
    """Divide uma carga histórica em lotes diários para evitar timeout no PostgreSQL/Streamlit Cloud."""
    ini = pd.to_datetime(data_inicio).date()
    fim = pd.to_datetime(data_fim).date()
    if fim < ini:
        return []
    lotes = []
    atual = ini
    dias_por_lote = max(1, int(dias_por_lote or 1))
    while atual <= fim:
        lote_fim = min(atual + timedelta(days=dias_por_lote - 1), fim)
        lotes.append((atual, lote_fim))
        atual = lote_fim + timedelta(days=1)
    return lotes


def _set_job_status(status, mensagem='', progresso=0, etapa='', registros_e=0, registros_v=0):
    set_config('update_job_status', status)
    set_config('update_job_message', mensagem)
    set_config('update_job_progress', int(progresso))
    set_config('update_job_stage', etapa)
    set_config('update_job_entries', int(registros_e or 0))
    set_config('update_job_sales', int(registros_v or 0))
    set_config('update_job_updated_at', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))


def obter_status_job_atualizacao():
    return {
        'status': get_config('update_job_status', 'Parado'),
        'mensagem': get_config('update_job_message', ''),
        'progresso': int(float(get_config('update_job_progress', '0') or 0)),
        'etapa': get_config('update_job_stage', ''),
        'entradas': int(float(get_config('update_job_entries', '0') or 0)),
        'vendas': int(float(get_config('update_job_sales', '0') or 0)),
        'atualizado_em': get_config('update_job_updated_at', ''),
    }


def atualizar_bases_historico_em_lotes(data_inicio=None, data_fim=None, tipo='MANUAL_BATCH', dias_por_lote=None):
    """Carga completa profissional: busca histórico em lotes e salva progresso.

    Em vez de fazer uma consulta de 500+ dias, o sistema executa períodos pequenos,
    substitui somente o intervalo do lote e grava o andamento. Assim o cache pode ser
    completado mesmo em ambientes limitados como Streamlit Cloud.
    """
    ini = pd.to_datetime(data_inicio or FULL_SYNC_START_DATE).date()
    fim = pd.to_datetime(data_fim or date.today()).date()
    lote_dias = int(dias_por_lote or CLOUD_SQL_MAX_DAYS or 1)
    lote_dias = max(1, min(lote_dias, 7))  # segurança: nunca executa lote histórico grande no Cloud
    lotes = dividir_periodos_em_lotes(ini, fim, lote_dias)
    if not lotes:
        raise RuntimeError('Período inválido para atualização.')

    total_e = 0
    total_v = 0
    _set_job_status('Rodando', f'Iniciando carga em {len(lotes)} lotes...', 0, f'{ini} a {fim}', 0, 0)
    registrar_atualizacao(tipo, 'Iniciado', 0, f'Carga em lotes iniciada: {ini} a {fim}. Lotes: {len(lotes)}.')

    for idx, (lote_ini, lote_fim) in enumerate(lotes, start=1):
        progresso = int(((idx - 1) / len(lotes)) * 100)
        _set_job_status('Rodando', f'Processando lote {idx}/{len(lotes)}: {lote_ini} a {lote_fim}', progresso, f'{lote_ini} a {lote_fim}', total_e, total_v)
        try:
            qtd_e, qtd_v, _, _ = atualizar_bases_periodo(lote_ini, lote_fim, tipo=f'{tipo}_LOTE_{idx:03d}')
            total_e += int(qtd_e or 0)
            total_v += int(qtd_v or 0)
            _set_job_status('Rodando', f'Lote {idx}/{len(lotes)} concluído.', int((idx / len(lotes)) * 100), f'{lote_ini} a {lote_fim}', total_e, total_v)
        except Exception as exc:
            msg = f'Erro no lote {idx}/{len(lotes)} ({lote_ini} a {lote_fim}): {exc}'
            _set_job_status('Erro', msg, progresso, f'{lote_ini} a {lote_fim}', total_e, total_v)
            registrar_atualizacao(tipo, 'Erro', total_e + total_v, msg)
            raise
    ...
    try:
        ...
        registrar_atualizacao(tipo, 'Aviso', total_e + total_v, f'Carga concluída, mas a apuração não foi recalculada automaticamente: {exc}')
    msg = f'Carga completa concluída: {total_e} entradas e {total_v} vendas em {len(lotes)} lotes.'
    _set_job_status('Concluído', msg, 100, f'{ini} a {fim}', total_e, total_v)
    registrar_atualizacao(tipo, 'Sucesso', total_e + total_v, msg)
    return total_e, total_v, pd.DataFrame(), pd.DataFrame()


def _worker_atualizacao_completa_em_lotes():
    try:
        atualizar_bases_historico_em_lotes(FULL_SYNC_START_DATE, date.today(), tipo='BACKGROUND_BATCH')
    except Exception as exc:
        _set_job_status('Erro', str(exc), int(float(get_config('update_job_progress', '0') or 0)), get_config('update_job_stage', ''), get_config('update_job_entries', 0), get_config('update_job_sales', 0))


def iniciar_atualizacao_completa_background():
    """Inicia a carga completa em segundo plano para liberar rapidamente a tela."""
    status = get_config('update_job_status', 'Parado')
    if status == 'Rodando':
        return False, 'Já existe uma atualização em andamento.'
    _set_job_status('Rodando', 'Atualização completa iniciada em segundo plano.', 0, 'Preparando...', 0, 0)
    t = threading.Thread(target=_worker_atualizacao_completa_em_lotes, daemon=True)
    t.start()
    return True, 'Atualização completa iniciada. Você pode acompanhar o progresso nesta tela.'


def atualizar_bases_completa_ate_hoje(tipo='AUTO_FULL_04H'):
    """Carga oficial completa em lotes, sem bloqueio por período longo."""
    return atualizar_bases_historico_em_lotes(FULL_SYNC_START_DATE, date.today(), tipo=tipo)
...
st.sidebar.markdown('---')
st.sidebar.caption('Versão 15.1 Cloud Lotes Diários')
...
if menu == 'Início':
    ...
    if aba_import == 'Atualização automática/cache':
        ...
        st.success('Atualização profissional ativada: a carga histórica é feita em lotes e a atualização diária é incremental. O cliente não precisa escolher período.')

        with st.form('form_auto_cache'):
            habilitado = st.checkbox('Ativar atualização automática: 04:00 carga completa em lotes + hora em hora somente o dia atual', value=get_config('auto_enabled', '0') == '1')
            c1, c2, c3 = st.columns(3)
            with c1:
            ...
            if salvar_cfg:
                ...
                st.success('Configuração salva. O sistema fará carga completa em lotes na primeira execução e depois atualização incremental.')

        status_job = obter_status_job_atualizacao()
        st.markdown('#### Atualização do banco de dados')
        cbtn, cinfo = st.columns([1, 3])
        with cbtn:
            if st.button('Atualizar Banco de Dados', type='primary', disabled=status_job.get('status') == 'Rodando'):
                ok, msg = iniciar_atualizacao_completa_background()
                if ok:
                    st.success(msg)
                else:
                    st.warning(msg)
        with cinfo:
            if st.button('Atualizar somente hoje'):
                try:
        ...
        status_job = obter_status_job_atualizacao()
        progresso = max(0, min(100, int(status_job.get('progresso', 0))))
        if status_job.get('status') == 'Rodando':
            st.info(status_job.get('mensagem') or 'Atualização em andamento...')
            st.progress(progresso)
            st.caption(f"Etapa: {status_job.get('etapa','')} | Entradas: {status_job.get('entradas',0)} | Vendas: {status_job.get('vendas',0)} | Atualizado em: {status_job.get('atualizado_em','')}")
            st.caption('A carga roda em lotes diários. Clique em Recarregar dados da tela para acompanhar o progresso.')
        elif status_job.get('status') == 'Concluído':
            st.success(status_job.get('mensagem') or 'Atualização concluída.')
            st.progress(100)
        elif status_job.get('status') == 'Erro':
            st.error(status_job.get('mensagem') or 'Erro na atualização.')
        else:
            st.caption('Nenhuma atualização completa em andamento.')
...
st.markdown('<div class="sb-footer">SB Farma | Projeto Negociação v15.0 Streamlit Cloud</div>', unsafe_allow_html=True)
