...
st.set_page_config(page_title='SB Farma | Projeto Negociação v14.3 Cloud Definitivo', page_icon='💊', layout='wide')
...
def instalar_guard_dom_cloud():
    """Proteção real contra Chrome Translate/extensões no Streamlit Cloud.

    O erro removeChild ocorre no React quando algo externo altera/removes nós do DOM.
    O CSS/HTML renderizado pelo Streamlit não chega ao <head> do documento principal.
    Por isso usamos um único componente invisível e estável, carregado uma vez, para
    marcar o documento pai inteiro como notranslate antes de qualquer atualização pesada.
    """
    if not CLOUD_MODE:
        return
    try:
...
(function(){
  function protect(){
    try {
      ...
      let style = d.getElementById('sb-farma-no-translate-style');
      if (!style) {
        style = d.createElement('style');
        style.id = 'sb-farma-no-translate-style';
        style.textContent = 'html,body,.stApp,[data-testid="stAppViewContainer"]{translate:no!important;}';
        d.head.appendChild(style);
      }
      const root = d.querySelector('[data-testid="stAppViewContainer"], .stApp');
      if (root) {
...
def executar_script_postgres(host, port, database, user, password):
    try:
        ...
        raise RuntimeError('Instale o pacote psycopg2-binary para importar direto do banco PostgreSQL.') from exc
...
def atualizar_cache_sql(tipo='MANUAL_SQL'):
    ...
    try:
        ...
        raise RuntimeError('Instale o pacote psycopg2-binary para conectar no PostgreSQL.') from exc
...
st.markdown('<div class="sb-footer">SB Farma | Projeto Negociação v14.3 Cloud Definitivo</div>', unsafe_allow_html=True)
