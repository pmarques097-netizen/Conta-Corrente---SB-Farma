...
CLOUD_SQL_MAX_DAYS = int(os.environ.get('SB_FARMA_CLOUD_SQL_MAX_DAYS', '31') or '31')
...
def validar_periodo_sql_cloud(data_inicio, data_fim, operacao='consulta SQL'):
    ...
    if CLOUD_MODE and dias > CLOUD_SQL_MAX_DAYS:
        raise RuntimeError(
            ...
            f'Use no máximo {CLOUD_SQL_MAX_DAYS} dias por atualização, ou execute a carga completa em servidor dedicado/local.'
        )

HOURLY_SYNC_MINUTE = 5  # roda uma vez por hora, próximo ao minuto 05

st.set_page_config(page_title='SB Farma | Projeto Negociação v15.0 Streamlit Cloud', page_icon='💊', layout='wide')
...
def executar_script_postgres(host, port, database, user, password):
    ...
    conn = psycopg2.connect(host=host, port=port, dbname=database, user=user, password=password, connect_timeout=8, options='-c statement_timeout=45000')
    try:
...
def dividir_periodos_em_lotes(data_inicio, data_fim, dias_por_lote=31):
    """Divide uma carga histórica em lotes pequenos para não derrubar o Streamlit Cloud."""
    ini = pd.to_datetime(data_inicio).date()
    fim = pd.to_datetime(data_fim).date()
    if fim < ini:
        return []
    lotes = []
    atual = ini
    dias_por_lote = max(1, int(dias_por_lote or 31))
    while atual <= fim:
...
def atualizar_bases_historico_em_lotes(data_inicio=None, data_fim=None, tipo='MANUAL_BATCH', dias_por_lote=None):
    ...
    lote_dias = int(dias_por_lote or min(CLOUD_SQL_MAX_DAYS, 31) or 31)
...
st.sidebar.markdown('---')
st.sidebar.caption('Versão 15.0 Cloud Profissional')
...
if menu == 'Início':
    ...
    if aba_import == 'Atualização automática/cache':
        ...
        if status_job.get('status') == 'Rodando':
            ...
            st.caption('A carga continua em segundo plano enquanto o app estiver ativo. Clique em Recarregar dados da tela para acompanhar.')
