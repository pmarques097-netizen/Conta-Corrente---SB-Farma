...
st.set_page_config(page_title='SB Farma | Projeto Negociação v14.9 Streamlit Cloud', page_icon='💊', layout='wide')
...
def atualizar_bases_completa_ate_hoje(tipo='AUTO_FULL_04H'):
    ...
    fim = date.today()
    validar_periodo_sql_cloud(ini, fim, 'carga completa')
...
st.sidebar.markdown('---')
st.sidebar.caption('Versão 14.9 Cloud Estável')
...
if menu == 'Início':
    ...
    if aba_import == 'Atualização automática/cache':
        ...
        if CLOUD_MODE:
            st.warning(f'No Streamlit Cloud, atualizações SQL longas são bloqueadas para evitar queda no health check. Limite atual: {CLOUD_SQL_MAX_DAYS} dias por consulta.')
        ...
        with col_a:
            if st.button('Atualizar entradas e vendas agora', type='primary'):
                try:
                    with st.spinner('Atualizando entradas e vendas...'):
        ...
        with col_b:
            st.caption('No Streamlit Cloud, a carga completa pode ser bloqueada por segurança. Use períodos curtos ou atualize somente o dia atual.')
...
st.markdown('<div class="sb-footer">SB Farma | Projeto Negociação v14.9 Streamlit Cloud</div>', unsafe_allow_html=True)
